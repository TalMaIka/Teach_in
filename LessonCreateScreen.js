...
/** Helpers — évite les décalages UTC */
...
function parseYYYYMMDDToLocalDate(yyyy_mm_dd) {
  const [y, m, d] = yyyy_mm_dd.split('-').map(Number);
  return new Date(y, (m || 1) - 1, d || 1, 12, 0, 0); // midi local pour éviter les bascules
}
...
export default function LessonScreen({ teacherId, navigation }) {
  ...
  const onDateChange = (_event, selectedDate) => {
    ...
    if (selectedDate) {
      const isoLocal = formatLocalYYYYMMDD(selectedDate);
      setDate(isoLocal);
      setErrors((e) => ({ ...e, date: '' }));
    }
  };
  ...
  const onTimeChange = (_event, selectedTime) => {
    ...
    if (selectedTime) {
      const h = pad2(selectedTime.getHours());
      const m = pad2(selectedTime.getMinutes());
      setTime(`${h}:${m}`);
      setErrors((e) => ({ ...e, time: '' }));
    }
  };
          ...
          >
            <View style={styles.card}>
              ...
              <TouchableOpacity
                  ...
                  activeOpacity={0.85}
                  style={[styles.input, styles.selectLike, errors.date && styles.inputError]}
              >
              ...
              {showDatePicker && (
                  <DateTimePicker
                      value={date ? parseYYYYMMDDToLocalDate(date) : new Date()}
                      mode="date"
              ...
              <PrimaryButton
                  ...
                  disabled={!title || !date || !isValidTime(time)}
                  loading={saving}
              />
              <HelperText>* required fields</HelperText>
            </View>
          </ScrollView>
        </KeyboardAvoidingView>
      </View>
